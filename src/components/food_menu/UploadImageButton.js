...
import {
  renderPreviewImage,
  renderUploadImage
} from "../../image_compressor/CompressImage";
...
class UploadImageButton extends Component {
  ...
  onDrop = async (acceptedFiles, rejectedFiles) => {
    if (acceptedFiles.length > 0) {
      ...
      try {
        const imageName =
          this.props.userName + "_" + this.props.idOfOffer + "_" + id;
        ...
        uploadTask.on(
          "state_changed",
          snapshot => this.uploadProgressHandler(snapshot, fileToUpload),
          err => this.uploadErrorHandler(err),
          () => this.uploadCompleteHandler(uploadTask)
        );
      } catch (e) {
        console.log(e);
      }
