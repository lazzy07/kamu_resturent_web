...
class MessagesScreen extends Component{
    ...
    changeScreen = () => {
        ...
        if(this.state.width>MIN_WIDTH_TO_MOBILE_VIEW)
            chatSender = (<ChatSender width={this.state.width*(parseInt(messageListWidth, 10)/100)}/>)
        ...
        if(this.state.messageList){
            ...
            if(messages.length%MESSAGE_LIST_LENGTH === 0){
                loadMoreMessagesButton = (
                    <div style={{paddingtop: '120px', textAlign: 'center', width: "100%", paddingBottom: "30px"}}>
                        <i className="fa fa-chevron-up"/>&nbsp;&nbsp;<b>Load More Messages</b>
                    </div>
                )
            }
            messageArray = (
            ...
            if(this.state.width < MIN_WIDTH_TO_MOBILE_VIEW){
                ...
                return (
                    <div style={{display: "flex"}}>
                        ...
                        <div style={{position: 'relative', margin: '0px', padding: '0px', width: messageListWidth+"%"}}>
                            {chatContent}
                        </div>
                    </div>
                )
