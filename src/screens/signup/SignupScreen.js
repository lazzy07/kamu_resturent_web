...
import { RESTURANT_LOGIN, RESTURANT_EMAIL_VERIFICATION } from '../../constants/routes';
...
class SignupScreen extends Component{
    ...
    constructor(props){
        ...
        this.state = {
            ...
            firstNameDescription: (<span style={this.descriptionStyles.normal}>Eg: Cyborg Studios Kandy</span>),
            userNamedescription: (<span style={this.descriptionStyles.normal}>Eg: @cyborg_studios_kandy</span>),
            enterpriseDescription: (<span style={this.descriptionStyles.normal}>eg: @cyborg_studios (If have one registered)</span>),
            emailDescription: (<span style={this.descriptionStyles.normal}>Eg: cyborgstudios@cyborg.com</span>),
            passwordDescription: (<span style={this.descriptionStyles.normal}>No spaces are allowed</span>),
            confirmPasswordDescription: (<span style={this.descriptionStyles.normal}></span>),
            cityDescription: (<span style={this.descriptionStyles.normal}>eg: Kandy</span>),
    ...
    sendSignupInfo = (data) => {
        try{
            axios.post(FIREBASE_FUNCTION_URL+'registerResturent', data, {headers:{'Content-Type': 'application/json;charset=UTF-8',"Access-Control-Allow-Origin": "*"}}).then(res => {
                this.setState({
                    loading: false,
                })
                ...
                if(res.status === 201){
                    this.setState({
                        mainMessage: (<span style={this.descriptionStyles.warning}>{res.data}</span>)
                    })
                }
                    ...
                    if(uName.length >= 4 && uName.length <= 15){
                        if(uName.replace("@", "").indexOf("@") === -1){
                            ...
                            if(re.test(uName.replace("@", ""))){
                                if(email.length>0){
                                    if(email.replace("@", "").indexOf("@") === -1 && email.indexOf("@") !== -1){
                                                ...
                                                if(this.state.confirmPassword === this.state.password){
                                                    if(eName.charAt(0) === '@' || eName=== ""){
                                                        if(city.length > 0){
                                                            ...
                                                            this.setState({
                                                                cityDescription: (<span style={this.descriptionStyles.warning}>City name can't be empty</span>)
                                                            })
                                                        }
                                                    }else{
                                                        this.setState({
                                                            enterpriseDescription: (<span style={this.descriptionStyles.warning}>Please enter a valid Enterprise Username</span>)
                                                        })
                                                    }
                                                }else{
                                                    this.setState({
                                                        confirmPasswordDescription: (<span style={this.descriptionStyles.warning}>Password didn't match</span>)
                                                    })
                                                }
                                            }else{
                                                this.setState({
                                                    confirmPasswordDescription: (<span style={this.descriptionStyles.warning}>This field can't be empty</span>)
                                                })
                                            }
                                        }else{
                                            this.setState({
                                                passwordDescription: (<span style={this.descriptionStyles.warning}>Password must be atleast 8 characters long</span>)
                                            })
                                        }
                                    }else{
                                        this.setState({
                                            emailDescription: (<span style={this.descriptionStyles.warning}>Plese enter a valid email address</span>)
                                        })
                                    }
                                }else{
                                    this.setState({
                                        emailDescription: (<span style={this.descriptionStyles.warning}>Email can't be empty</span>)
                                    })
                                }
                            }else{
                                this.setState({
                                    userNamedescription: (<span style={this.descriptionStyles.warning}>Username can only consists of '_', '@' at the front, letters and numbers  eg: @cyborgstudio_7</span>)
                                })
                            }
                        }else{
                            this.setState({
                                userNamedescription: (<span style={this.descriptionStyles.warning}>Username can only consists of one @ at the front eg: @lazzy_07</span>)
                            });
                        }
                    }else{
                        this.setState({
                            userNamedescription: (<span style={this.descriptionStyles.warning}>Username must be between 4-15 characters</span>)
                        })
                    }
                }else{
                    this.setState({
                        userNamedescription: (<span style={this.descriptionStyles.warning}>Username must start with '@' eg: @lazzy07</span>)
                    })
                }   
            }else{
                this.setState({
                    firstNameDescription: (<span style={this.descriptionStyles.warning}>First name can't be empty</span>)
                })
            }
        }
    }
            ...
            <Animated animationIn='fadeIn' isVisible={true}>
                <div style={{paddingBottom: '10px'}}>
                    ...
                    >
                        <div>
                            <Animated animationIn='fadeIn' isVisible={true}>
                                <div style={{color: '#7a7a7a', paddingTop: '20px',paddingBottom: '30px', textAlign: 'center'}}>
                                    <h1>Kamu <span style={{color: '#e8232d'}}>Signup</span></h1>
                        ...
                        <div>
                            <form>
                                <Animated animationIn='fadeInRight' animationInDelay={50} isVisible={true}>
                                    <InputBox 
                                        name="firstName"
                                        label="Enter Name*" 
                                        type="text"
                                ...
                                <Animated animationIn='fadeInRight' animationInDelay={100} isVisible={true}>
                                    <InputBox
                                        name="email"
                                        label="Enter Email*" 
                                        type="email" 
                                ...
                                <Animated animationIn='fadeInRight' animationInDelay={150} isVisible={true}>
                                    <InputBox 
                                        name="userName"
